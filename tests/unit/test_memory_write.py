...
async def test_new_subject_creates_note_concept(write_settings: Settings) -> None:
    ...
    assert '"type": "Note"' in insert_form["$rows"]
    assert '"trust": 1.0' in insert_form["$rows"]
    assert '"source_query": "SELECT 1;"' in insert_form["$rows"]
...
async def test_walk_owned_concept_gets_annotated_overlay(write_settings: Settings) -> None:
    ...
    assert '"valid_to"' in upsert_form["$rows"]
    assert (
        "- old note\\n\\n- `price` is in cents" in insert_form["$rows"].replace("\\u0060", "`")
        or "- old note" in insert_form["$rows"]
    )
    assert '"core": "# Schema' in insert_form["$rows"]
...
async def test_cc_client_write_guard_rejects_foreign_statements(
    ...
    with pytest.raises(GatewayError) as excinfo:
        await disabled.client.execute_memory_write(
            "INSERT INTO Dashboard.Memory ($rows);", client_context_id="sess::t::1"
        )
    assert excinfo.value.error_type is ErrorType.FORBIDDEN
