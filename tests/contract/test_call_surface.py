...
async def test_recommend_indexes_call_uses_native_advise() -> None:
    ddl = "CREATE INDEX idx_adv_city ON `Default`.`S`.`Orders`(city)"
    advise = {
        "status": "success",
        "results": [
            {
                "advice": {
                    "adviseinfo": {
                        "current_indexes": [],
                        "recommended_indexes": {"indexes": [{"index_statement": ddl}]},
                    }
                }
            }
        ],
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        if statement.startswith("ADVISE"):
            return httpx.Response(200, json=advise)
        return httpx.Response(200, json={"status": "success", "results": []})
